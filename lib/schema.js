...
const SchemaColumnError = require('./error/schemaColumnError');
const SchemaColumnMissingKey = require('./error/schemaColumnMissingKey');
const SchemaColumnTypeError = require('./error/schemaColumnTypeError');
const SchemaColumnInvalidKey = require('./error/schemaColumnInvalidKeys');
  ...
  static isValidSchemaObj(obj) {
    ...
    if (!util.isObject(obj)) {
      throw new CheetahError(
        'Invalid input `obj`. Input `obj` must be of type Object.'
      );
    }

    // 2) check that each value in the schema is an object
    for (let column of Object.keys(obj)) {
      if (!util.isObject(obj[column])) {
        throw new SchemaColumnError(column);
      }
    }
    ...
    for (let column of Object.keys(obj)) {
      if (!obj[column].type) {
        throw new SchemaColumnMissingKey(column, 'type');
      }
    }

    // 4) check that each type keys value is one of the valid types defined in cheetah.types
    for (let column of Object.keys(obj)) {
      const typeName = obj[column].type.name;
      if (!typeDict[typeName]) {
        throw new SchemaColumnTypeError(column, typeName);
      }
    }
    ...
    for (let column of Object.keys(obj)) {
      const keys = new Set(Object.keys(obj[column]));
      const difference = utils.difference(keys, whitelist);
      const invalidKeys = [...difference];
      if (invalidKeys.length > 0) {
        throw new SchemaColumnInvalidKey(column, invalidKeys, whitelist);
      }
    }
  }
